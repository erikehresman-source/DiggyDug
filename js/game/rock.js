export class Rock {
  ...
  update(grid, entities, dt){
    ...
    if (emptyBelow){
      this.fallDelay += dt;
      if (this.fallDelay>0.25) { this.falling=true; }
    } else {
      // roll if supported diagonally
      ...
      if (canLeft){ this.x-= dt*6; }
      else if (canRight){ this.x+= dt*6; }
      this.fallDelay = 0;
      this.falling=false; this.vy=0;
    }
    if (this.falling){
      this.vy += 40*dt; this.y += this.vy*dt;
      // crush enemies under rock
      for(const e of entities){
        if(!e.alive || e.dead) continue;
        if (Math.abs(e.x-this.x)<0.4 && e.y>this.y && Math.abs(e.y - this.y) < 1.0){
          e.alive=false; e.dead=true;
        }
      }
      // land if ground
      if (grid.get(Math.round(this.x), Math.floor(this.y)+1)===1){
        this.falling=false; this.vy=0;
      }
    }
  }
}
function blocked(entities, x,y){
  return entities.some(e=>!e.dead && Math.round(e.x)===x && Math.round(e.y)===y);
}
